...
export class MapaGlobalEncuestas extends Component {
  ...
  get filteredEncuestas() {
    ...
    return (this.state.encuestas || []).filter((encuesta) => {
      return (
        cleanText(encuesta.cliente).includes(term) ||
  ...
  async initMap() {
    ...
    L.Icon.Default.mergeOptions({
      iconRetinaUrl:
        "/encuesta_root/static/src/libs/leaflet/images/marker-icon-2x.png",
      iconUrl: "/encuesta_root/static/src/libs/leaflet/images/marker-icon.png",
      shadowUrl:
        "/encuesta_root/static/src/libs/leaflet/images/marker-shadow.png",
    });
    ...
    this.state.encuestas = encuestas.map((encuesta) => {
      let nombreMunicipio = "";
      if (Array.isArray(encuesta.municipio_id))
        nombreMunicipio = encuesta.municipio_id[1];
      else if (encuesta.municipio_id) nombreMunicipio = encuesta.municipio_id;

      const etiqueta =
        sistemasDiccionario[encuesta.sistema_recomendado] ||
        encuesta.sistema_recomendado ||
        "No definido";
      ...
      if (encuesta.latitud && encuesta.longitud) {
        ...
        marker.on("popupopen", () => {
          const btn = document.querySelector(
            `.open-encuesta-btn-popup[data-id="${encuesta.id}"]`,
          );
          if (btn) {
  ...
  drawHeatmapMode() {
    ...
    this.heatmapLayer = L.heatLayer(heatPoints, {
      ...
      gradient: {
        // Paleta elegante de colores térmicos
        0.4: "blue",
        0.6: "cyan",
        0.7: "lime",
        0.8: "yellow",
...
registry
  .category("actions")
  .add("action_mapa_global_encuestas", MapaGlobalEncuestas);
